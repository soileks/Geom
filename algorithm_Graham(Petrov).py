...
def rotate(p1, p2, p3):
    return (p2[0] - p1[0]) * (p3[1] - p2[1]) - (p2[1] - p1[1]) * (p3[0] - p2[0]) # вычисляем det векторов
...
def grahamScan(N):
    ...
    t = min(N, key=lambda point: (point[0], point[1]))
    N.remove(t)#удаляем точку t из множества

    def compare(p1, p2): # сравнение векторов
        # Координаты вектора v1
        v1x = p1[0] - t[0]
        v1y = p1[1] - t[1]
        ...
        if det > 0:
            ...
            # v1 == v2 по определителю. Сравним по длине
    ...
    for i in range(1, len(N)):
        ...
        if rot == 0:
            ...
            # Пока p1, p2, p3 образуют правый поворот - исключаем p2  (кратчайший поворот -  правый)(против часовой стрелки, если смотреть сверху)
...
def readF():
    ...
    f = open('C:\\Users\\User\\source\\repos\\Наивный и алгоритм Рэма для питона\\result.txt')
...
def main():
    ...
    V = grahamScan(N)
    drawResult(N, V, title="Выпуклая оболочка:\n")
